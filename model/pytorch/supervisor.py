...
from model.pytorch.model import BGSLFModel
...
class BGSLFSupervisor:
    def __init__(self, args):
        ...
        # setup model
        BGSLF_model = BGSLFModel(self._train_feas, self.temperature, self._logger, args)
        self.BGSLF_model = BGSLF_model.to(self.device)
    ...
    def save_model(self, epoch):
        ...
        config = {}
        config['model_state_dict'] = self.BGSLF_model.state_dict()
        config['epoch'] = epoch
    ...
    def load_model(self):
        ...
        self.BGSLF_model.load_state_dict(checkpoint['model_state_dict'])
        self._logger.info("Loaded model at {}".format(self._epoch_num))

    def _setup_graph(self):
        with torch.no_grad():
            self.BGSLF_model = self.BGSLF_model.eval()

            val_iterator = self._data['val_loader'].get_iterator()

            for _, (x, y) in enumerate(val_iterator):
                x, y = self._prepare_data(x, y)
                output = self.BGSLF_model(x)
                break
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (x, y) in enumerate(val_iterator):
                ...
                output, mid_output = self.BGSLF_model(x, temp)
    ...
    def _train(self, args):
        ...
        if self.opt == 'adam':
            optimizer = torch.optim.Adam(self.BGSLF_model.parameters(), lr=base_lr, eps=epsilon)
        elif self.opt == 'sgd':
            optimizer = torch.optim.SGD(self.BGSLF_model.parameters(), lr=base_lr)
        else:
            optimizer = torch.optim.Adam(self.BGSLF_model.parameters(), lr=base_lr, eps=epsilon)
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            self.BGSLF_model = self.BGSLF_model.train()
            ...
            for batch_idx, (x, y) in enumerate(train_iterator):
                optimizer.zero_grad()
                x, y = self._prepare_data(x, y)
                output, mid_output = self.BGSLF_model(x, temp, y, batches_seen)
                ...
                self.BGSLF_model.to(self.device)
                ...
                torch.nn.utils.clip_grad_norm_(self.BGSLF_model.parameters(), self.max_grad_norm)
